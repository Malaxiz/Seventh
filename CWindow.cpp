...
int CWindow::onInit(std::string title, int width, int height) {
    ...
    if(_window == nullptr) {
        CLocator::getService<SFile>()->log(LogType::ERROR, "At SDL_CreateWindow: ", SDL_GetError());
        return -1;
    }
    ...
    if(_renderer == nullptr) {
        CLocator::getService<SFile>()->log(LogType::ERROR, "At SDL_CreateRenderer: ", SDL_GetError());
        return -1;
    }
